   ...
   public static void main(String[] args) {
    ...
    public static void haeJunatAsemanMukaan() {
        ...
        for (Juna j : asemaJunat) {
            if (j.timeTableRows.get(0).getType().equals("DEPARTURE") && j.timeTableRows.get(0).getStationShortCode().equals(annettuAsema)) {
                System.out.println(
                        "Lähtevät: " + j.getTrainType() + j.getTrainNumber()
                        + " Pääteasema: " + j.timeTableRows.get(j.timeTableRows.size()-1).getStationShortCode()
                        + " Lähtöaika: " + j.timeTableRows.get(0).getScheduledTime()
        ...
        for (Juna j: asemaJunat) {
            ...
            if(j.timeTableRows.get(j.timeTableRows.size()-1).getType().equals("ARRIVAL")) {
                System.out.println(
                        "Saapuvat: " + j.getTrainType() + j.getTrainNumber()
                                + " Lähtöasema: " + j.timeTableRows.get(0).getStationShortCode()
                                + " Saapumisaika: " + j.timeTableRows.get(j.timeTableRows.size()-1).getScheduledTime()
                                + " Junan tunnus, jos lähijuna: " + j.getCommuterLineID());
            }
        }
    }
